#!/bin/bash
# Script to update old IDs to new readable format

# Find all TypeScript, TSX, and MD files in src and test directories
FILES=$(find src test -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.md" \) ! -path "*/node_modules/*")
...
for file in $FILES; do
  ...
  # Financial situation IDs (note: some files incorrectly use FS for financial)
...
echo "✅ Updated IDs in all source files"
