...
function generateErscheinungsbildProse(
  ...
  // Pflegezustand: "Der Proband wirkte gepflegt (sauber). Extra text."
  if (items.pflegezustand.length > 0) {
    let pending: string[] = [];
    for (const item of items.pflegezustand) {
      if (item.includes('. ')) {
        if (pending.length > 0) {
          sentences.push(`${p.derDie} Proband wirkte ${formatListWithUnd(pending.map(s => s.toLowerCase()))}`);
          pending = [];
        }
        sentences.push(...processItemWithExtraText(item, `${p.derDie} Proband wirkte `));
      } else {
        pending.push(item);
      }
    }
  ...
  // Kleidungsangemessenheit: "Die Kleidung war angemessen."
  if (items.kleidungsangemessenheit.length > 0) {
    let pending: string[] = [];
    for (const item of items.kleidungsangemessenheit) {
      if (item.includes('. ')) {
        if (pending.length > 0) {
          sentences.push(`Die Kleidung war zur Umgebungstemperatur ${formatListWithUnd(pending.map(s => s.toLowerCase()))}`);
          pending = [];
        }
        sentences.push(...processItemWithExtraText(item, 'Die Kleidung war zur Umgebungstemperatur '));
      } else {
        pending.push(item);
      }
    }
...
function generateKontaktverhaltenSentence(data: Kontaktverhalten | undefined, p: GenderPronouns): string | null {
  ...
  return `${p.derDie} zeigte sich ${formatListWithUnd(items.map(lowercaseFirst))} im Kontakt.`;
}
  ...
  for (const item of data.halluzinationen) {
    ...
    if (item.includes('/')) {
      ...
      for (const part of parts) {
        // p.derDie already contains "Der Patient" / "Die Patientin"
        sentences.push(`${p.derDie} berichtete von ${part}en Halluzinationen`);
      }
    } else if (['akustisch', 'visuell'].includes(item.toLowerCase())) {
      // Single adjective: add -en ending (Dativ) + "Halluzinationen"
      sentences.push(`${p.derDie} berichtete von ${item}en Halluzinationen`);
    } else {
      // "sonstige Sinneswahrnehmungen" - needs dative: "sonstigen Sinneswahrnehmungen"
      const dativeForm = item.replace('sonstige ', 'sonstigen ');
      sentences.push(`${p.derDie} berichtete von ${dativeForm}`);
    }
  }
