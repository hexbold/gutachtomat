...
/** Toggle value in Partial<Record<Enum, 'selected'>> selection (immutably) */
function toggleSelectionValue(obj: Record<string, unknown>, path: string, value: string): Record<string, unknown> {
  ...
  if (currentSelection[value] === 'selected') {
    const { [value]: _, ...rest } = currentSelection;
    return setNestedValue(obj, path, rest);
  } else {
    return setNestedValue(obj, path, { ...currentSelection, [value]: 'selected' });
  }
}
...
export function formReducer(state: FormTypes.GutachtenFormState, action: FormTypes.FormAction): FormTypes.GutachtenFormState {
  switch (action.type) {
    ...
    case 'CONFIRM_STEP3_SUBSTEP':
      return {
        ...state,
        wizardState: {
          ...state.wizardState,
          step3ConfirmedSubSteps: new Set([...state.wizardState.step3ConfirmedSubSteps, action.substep]),
        },
      };

    case 'COMPLETE_STEP3_SUBSTEP':
      return {
        ...state,
        wizardState: {
          ...state.wizardState,
          step3CompletedSubSteps: new Set([...state.wizardState.step3CompletedSubSteps, action.substep]),
        },
      };

    case 'CONFIRM_AMDP_PAGE':
      return {
        ...state,
        wizardState: {
          ...state.wizardState,
          amdpConfirmedPages: new Set([...state.wizardState.amdpConfirmedPages, action.page]),
        },
      };

    case 'COMPLETE_AMDP_PAGE':
      return {
        ...state,
        wizardState: {
          ...state.wizardState,
          amdpCompletedPages: new Set([...state.wizardState.amdpCompletedPages, action.page]),
        },
      };

    case 'CONFIRM_STEP4_SUBSTEP':
      return {
        ...state,
        wizardState: {
          ...state.wizardState,
          step4ConfirmedSubSteps: new Set([...state.wizardState.step4ConfirmedSubSteps, action.substep]),
        },
      };

    case 'COMPLETE_STEP4_SUBSTEP':
      return {
        ...state,
        wizardState: {
          ...state.wizardState,
          step4CompletedSubSteps: new Set([...state.wizardState.step4CompletedSubSteps, action.substep]),
        },
      };

    case 'CONFIRM_STEP5_SUBSTEP':
      return {
        ...state,
        wizardState: {
          ...state.wizardState,
          step5ConfirmedSubSteps: new Set([...state.wizardState.step5ConfirmedSubSteps, action.substep]),
        },
      };

    case 'COMPLETE_STEP5_SUBSTEP':
      return {
        ...state,
        wizardState: {
          ...state.wizardState,
          step5CompletedSubSteps: new Set([...state.wizardState.step5CompletedSubSteps, action.substep]),
        },
      };
