...
import { VERHALTENSBEZOGENE_FAKTOREN_LABELS } from '@/lib/core/form-labels';
import { MakroanalyseHandlers } from '@/hooks/useGutachtenForm';
import { useState, useEffect } from 'react';
import { createPortal } from 'react-dom';

interface VerhaltensbezogeneFaktorenModalProps {
  isOpen: boolean;
  onClose: () => void;
  data: FormTypes.VerhaltensbezogeneFaktorenData;
  onUpdate: MakroanalyseHandlers['updateVerhaltensbezogeneFaktoren'];
}

export function VerhaltensbezogeneFaktorenModal({
  isOpen,
  onClose,
  data,
  onUpdate
}: VerhaltensbezogeneFaktorenModalProps) {
  ...
  const handleToggle = (item: FormTypes.VerhaltensbezogeneFaktoren) => {
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Verhaltensbezogene Faktoren</h2>
          <button
            onClick={onClose}
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4 space-y-6">
          <div>
            <label className="block text-base font-bold text-gray-900 mb-3">Verhaltensbezogene Faktoren</label>
            <div className="grid grid-cols-2 gap-2.5">
              {Object.values(FormTypes.VerhaltensbezogeneFaktoren).map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`text-sm font-semibold leading-snug pr-6 ${isSelected ? 'text-blue-700' : 'text-gray-700'}`}>
                      {VERHALTENSBEZOGENE_FAKTOREN_LABELS[item]}
                    </span>
                  </button>
                );
              })}
          ...
          {/* Andere */}
          <div>
            <label className="block text-base font-bold text-gray-900 mb-2">Andere verhaltensbezogene Faktoren</label>
            <textarea
              value={data.andere}
              onChange={(e) => onUpdate({ andere: e.target.value })}
              rows={3}
              placeholder="Weitere verhaltensbezogene Faktoren..."
