...
interface SuizidalitaetModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function SuizidalitaetModal({
  isOpen,
  onClose,
  formData,
  befundHandlers,
}: SuizidalitaetModalProps) {
  ...
  const data = formData.suizidalitaet;

  const allSelected = [
    ...data.gradDerSuizidalitat,
    ...data.paktAbspracheFahigkeit,
    ...data.abklarungVonSuizidalitat,
  ];
  ...
  const handleClearAll = () => {
    befundHandlers.clearSuizidalitaet();
    handleClose();
  };
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl border-2 border-blue-200 dark:border-blue-800 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900 border-b-2 border-blue-200 dark:border-blue-800 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900 dark:text-blue-100">Suizidalität</h2>
        ...
        <div className="px-6 pt-4 pb-2">
          <div className="relative">
            <input
              ...
              placeholder="Suizidalität durchsuchen..."
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-8">
          ...
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-3">Grad der Suizidalität</h3>
            <div className="grid grid-cols-2 gap-2">
              {suizidalitaetGradOptions.filter(filterBySearch).map((option) => (
                <SymptomCard
                  key={option}
                  label={option}
                  highlightedLabel={highlight(option)}
                  isSelected={data.gradDerSuizidalitat.includes(option)}
                  onClick={() => befundHandlers.toggle('suizidalitaet', 'gradDerSuizidalitat', option)}
                />
              ))}
            </div>
          </div>

          {/* Pakt/Absprachefähigkeit */}
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-3">Pakt/Absprachefähigkeit</h3>
            <div className="grid grid-cols-2 gap-2">
              {suizidalitaetPaktAbspracheOptions.filter(filterBySearch).map((option) => (
                <SymptomCard
                  key={option}
                  label={option}
                  highlightedLabel={highlight(option)}
                  isSelected={data.paktAbspracheFahigkeit.includes(option)}
                  onClick={() => befundHandlers.toggle('suizidalitaet', 'paktAbspracheFahigkeit', option)}
                />
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-3">Abklärung von Suizidalität</h3>
            <div className="grid grid-cols-2 gap-2">
              {suizidalitaetAbklaerungOptions.filter(filterBySearch).map((option) => (
                <SymptomCard
                  key={option}
                  label={option}
                  highlightedLabel={highlight(option)}
                  isSelected={data.abklarungVonSuizidalitat.includes(option)}
                  onClick={() => befundHandlers.toggle('suizidalitaet', 'abklarungVonSuizidalitat', option)}
                />
              ))}
            </div>
          </div>
