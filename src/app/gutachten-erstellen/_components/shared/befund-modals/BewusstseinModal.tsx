...
interface BewusstseinModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function BewusstseinModal({
  isOpen,
  onClose,
  formData,
  befundHandlers,
}: BewusstseinModalProps) {
  ...
  const data = formData.bewusstsein;

  const allSelected = [
    ...data.quantitativesBewusstsein,
    ...data.qualitativesBewusstsein,
  ];
  ...
  const handleClearAll = () => {
    befundHandlers.clearBewusstsein();
    handleClose();
  };
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white dark:bg-gray-900 rounded-xl shadow-2xl border-2 border-blue-200 dark:border-blue-800 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900 border-b-2 border-blue-200 dark:border-blue-800 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900 dark:text-blue-100">Bewusstsein</h2>
        ...
        <div className="px-6 pt-4 pb-2">
          <div className="relative">
            <input
              ...
              placeholder="Bewusstsein durchsuchen..."
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-8">
          ...
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-3">Quantitatives Bewusstsein</h3>
            <div className="grid grid-cols-2 gap-2">
              {bewusstseinQuantitativOptions.filter(filterBySearch).map((option) => (
                <SymptomCard
                  key={option}
                  label={option}
                  highlightedLabel={highlight(option)}
                  isSelected={data.quantitativesBewusstsein.includes(option)}
                  onClick={() => befundHandlers.toggle('bewusstsein', 'quantitativesBewusstsein', option)}
                />
              ))}
            </div>
          </div>
          ...
          <div>
            <h3 className="text-lg font-semibold text-foreground mb-3">Qualitatives Bewusstsein</h3>
            <div className="grid grid-cols-2 gap-2">
              {bewusstseinQualitativOptions.filter(filterBySearch).map((option) => (
                <SymptomCard
                  key={option}
                  label={option}
                  highlightedLabel={highlight(option)}
                  isSelected={data.qualitativesBewusstsein.includes(option)}
                  onClick={() => befundHandlers.toggle('bewusstsein', 'qualitativesBewusstsein', option)}
                />
              ))}
            </div>
          </div>
