...
interface SonstigeSymptomatikModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function SonstigeSymptomatikModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: SonstigeSymptomatikModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  const [expandedKrankheit, setExpandedKrankheit] = useState<Set<FormTypes.KrankheitseinsichtComplianceSymptom>>(new Set());

  useEffect(() => {
    setMounted(true);
  }, []);

  const so = formData.sonstigeSymptomatik;
  const allSelectedSymptoms = getFormattedSonstigeSymptomatik(so).sort((a, b) => a.localeCompare(b));
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Sonstige Symptomatik</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            {renderCardSection(
              ...
              FormTypes.KrankheitseinsichtComplianceSymptom,
              KRANKHEITSEINSICHT_COMPLIANCE_LABELS,
              'sonstigeSymptomatik.krankheitseinsichtCompliance',
              so.krankheitseinsichtCompliance,
              expandedKrankheit,
              setExpandedKrankheit
            )}
            ...
            {(filterBySearch('Andere') || filterBySearch('Symptome')) && (
              <div>
                ...
                <textarea
                  value={so.andereSymptome}
                  onChange={(e) => setNestedField('sonstigeSymptomatik.andereSymptome', e.target.value)}
                  rows={3}
                  placeholder="Weitere sonstige Symptome..."
