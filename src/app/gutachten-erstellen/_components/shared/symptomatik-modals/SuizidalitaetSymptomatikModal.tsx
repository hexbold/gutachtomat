...
interface SuizidalitaetSymptomatikModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function SuizidalitaetSymptomatikModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: SuizidalitaetSymptomatikModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  const [expandedSymptome, setExpandedSymptome] = useState<Set<FormTypes.SuizidalitaetSymptomatikSymptom>>(new Set());

  useEffect(() => {
    setMounted(true);
  }, []);

  const ss = formData.suizidalitaetSymptomatik;
  const allSelectedSymptoms = getFormattedSuizidalitaetSymptomatik(ss).sort((a, b) => a.localeCompare(b));
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Suizidalität</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            {renderCardSection(
              'Suizidalität',
              FormTypes.SuizidalitaetSymptomatikSymptom,
              SUIZIDALITAET_SYMPTOMATIK_LABELS,
              'suizidalitaetSymptomatik.symptome',
              ss.symptome,
              expandedSymptome,
              setExpandedSymptome
            )}
            ...
            {(filterBySearch('Andere') || filterBySearch('Symptome')) && (
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setNestedField('suizidalitaetSymptomatik.andereSymptome', e.target.value)}
                  rows={3}
