...
interface SchlafstoerungsModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function SchlafstoerungsModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: SchlafstoerungsModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  // Expanded states for CardSelection sections
  const [expandedInsomnie, setExpandedInsomnie] = useState<Set<FormTypes.SchlafInsomniesymptom>>(new Set());
  const [expandedHypersomnie, setExpandedHypersomnie] = useState<Set<FormTypes.SchlafHypersomniesymptom>>(new Set());
  const [expandedRhythmus, setExpandedRhythmus] = useState<Set<FormTypes.SchlafRhythmusStoerung>>(new Set());
  const [expandedParasomnie, setExpandedParasomnie] = useState<Set<FormTypes.SchlafParasomniesymptom>>(new Set());

  useEffect(() => {
    setMounted(true);
  }, []);

  const ss = formData.nichtorganischeSchlafstoerungen;

  // Use shared utility for footer display
  const allSelectedSymptoms = getFormattedSchlafstoerungen(ss).sort((a, b) => a.localeCompare(b));
  ...
  const resetAllExpanded = () => {
    setExpandedInsomnie(new Set());
    setExpandedHypersomnie(new Set());
    setExpandedRhythmus(new Set());
    setExpandedParasomnie(new Set());
  };
  ...
  const handleClearAndClose = () => {
    symptomHandlers.clearNichtorganischeSchlafstoerungen();
    handleClose();
  };
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Nichtorganische Schlafstörungen</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            ...
            {renderCardSection(
              'Insomnie',
              FormTypes.SchlafInsomniesymptom,
              SCHLAF_INSOMNIE_LABELS,
              'nichtorganischeSchlafstoerungen.insomnie',
              ss.insomnie,
              expandedInsomnie,
              setExpandedInsomnie
            )}
            ...
            {renderCardSection(
              'Hypersomnie',
              FormTypes.SchlafHypersomniesymptom,
              SCHLAF_HYPERSOMNIE_LABELS,
              'nichtorganischeSchlafstoerungen.hypersomnie',
              ss.hypersomnie,
              expandedHypersomnie,
              setExpandedHypersomnie
            )}

            {/* Schlaf-Wach-Rhythmusstörungen */}
            {renderCardSection(
              'Schlaf-Wach-Rhythmusstörungen',
              FormTypes.SchlafRhythmusStoerung,
              SCHLAF_RHYTHMUS_LABELS,
              'nichtorganischeSchlafstoerungen.rhythmus',
              ss.rhythmus,
              expandedRhythmus,
              setExpandedRhythmus
            )}
            ...
            {renderCardSection(
              'Parasomnien',
              FormTypes.SchlafParasomniesymptom,
              SCHLAF_PARASOMNIE_LABELS,
              'nichtorganischeSchlafstoerungen.parasomnie',
              ss.parasomnie,
              expandedParasomnie,
              setExpandedParasomnie
            )}

            {/* Andere Symptome */}
            {filterBySearch('Andere') && (
              <div>
                <h3 className="text-lg font-bold text-gray-900 mb-4">{highlight('Andere Schlafstörungen')}</h3>
                <textarea
                  value={ss.andereSymptome}
                  onChange={(e) => setNestedField('nichtorganischeSchlafstoerungen.andereSymptome', e.target.value)}
                  ...
                  placeholder="Weitere Schlafstörungen beschreiben..."
                />
              </div>
            )}
          </div>
