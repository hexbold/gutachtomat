...
interface SomatoformeSymptomatikModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function SomatoformeSymptomatikModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: SomatoformeSymptomatikModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  // Expanded states for CardSelection sections
  const [expandedKoerperlich, setExpandedKoerperlich] = useState<Set<FormTypes.SomatoformKoerperlichSymptom>>(new Set());
  const [expandedAutonom, setExpandedAutonom] = useState<Set<FormTypes.SomatoformAutonomSymptom>>(new Set());
  const [expandedHypochondrisch, setExpandedHypochondrisch] = useState<Set<FormTypes.SomatoformHypochondrischSymptom>>(new Set());
  const [expandedSchmerz, setExpandedSchmerz] = useState<Set<FormTypes.SomatoformSchmerzSymptom>>(new Set());
  const [expandedKonversion, setExpandedKonversion] = useState<Set<FormTypes.SomatoformKonversionSymptom>>(new Set());

  useEffect(() => {
    setMounted(true);
  }, []);

  const ss = formData.somatoformeSymptomatik;

  // Use shared utility for footer display
  const allSelectedSymptoms = getFormattedSomatoformeSymptoms(ss).sort((a, b) => a.localeCompare(b));
  ...
  const resetAllExpanded = () => {
    setExpandedKoerperlich(new Set());
    setExpandedAutonom(new Set());
    setExpandedHypochondrisch(new Set());
    setExpandedSchmerz(new Set());
    setExpandedKonversion(new Set());
  };
  ...
  const handleClearAndClose = () => {
    symptomHandlers.clearSomatoformeSymptomatik();
    handleClose();
  };
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Somatoforme Symptomatik</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            ...
            {renderCardSection(
              'Körperliche Symptome',
              FormTypes.SomatoformKoerperlichSymptom,
              SOMATOFORM_KOERPERLICH_LABELS,
              'somatoformeSymptomatik.koerperlicheSymptome',
              ss.koerperlicheSymptome,
              expandedKoerperlich,
              setExpandedKoerperlich
            )}

            {/* Autonome Symptome */}
            {renderCardSection(
              'Autonome Symptome',
              FormTypes.SomatoformAutonomSymptom,
              SOMATOFORM_AUTONOM_LABELS,
              'somatoformeSymptomatik.autonomeSymptome',
              ss.autonomeSymptome,
              expandedAutonom,
              setExpandedAutonom
            )}
            ...
            {renderCardSection(
              'Hypochondrische Symptome',
              FormTypes.SomatoformHypochondrischSymptom,
              SOMATOFORM_HYPOCHONDRISCH_LABELS,
              'somatoformeSymptomatik.hypochondrischeSymptome',
              ss.hypochondrischeSymptome,
              expandedHypochondrisch,
              setExpandedHypochondrisch
            )}

            {/* Schmerz-Symptome */}
            {renderCardSection(
              'Schmerz-Symptome',
              FormTypes.SomatoformSchmerzSymptom,
              SOMATOFORM_SCHMERZ_LABELS,
              'somatoformeSymptomatik.schmerzSymptome',
              ss.schmerzSymptome,
              expandedSchmerz,
              setExpandedSchmerz
            )}
            ...
            {renderCardSection(
              'Konversions-Symptome',
              FormTypes.SomatoformKonversionSymptom,
              SOMATOFORM_KONVERSION_LABELS,
              'somatoformeSymptomatik.konversionSymptome',
              ss.konversionSymptome,
              expandedKonversion,
              setExpandedKonversion
            )}
            ...
            {filterBySearch('Andere') && (
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setNestedField('somatoformeSymptomatik.andereSymptome', e.target.value)}
                  ...
                  placeholder="Weitere somatoforme Symptome beschreiben..."
                />
              </div>
            )}
          </div>
