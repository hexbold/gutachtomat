...
interface SexuellbezogeneSymptomeModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function SexuellbezogeneSymptomeModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: SexuellbezogeneSymptomeModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  // Expanded states for CardSelection sections
  const [expandedFunktion, setExpandedFunktion] = useState<Set<FormTypes.SexuelleFunktionsstoerungSymptom>>(new Set());
  const [expandedPraeferenz, setExpandedPraeferenz] = useState<Set<FormTypes.SexualpraeferenzstoerungSymptom>>(new Set());

  useEffect(() => {
    setMounted(true);
  }, []);

  const sx = formData.sexuellbezogeneSymptome;

  // Use shared utility for footer display
  const allSelectedSymptoms = getFormattedSexuellbezogeneSymptome(sx).sort((a, b) => a.localeCompare(b));
  ...
  const resetAllExpanded = () => {
    setExpandedFunktion(new Set());
    setExpandedPraeferenz(new Set());
  };
  ...
  const handleClearAndClose = () => {
    symptomHandlers.clearSexuellbezogeneSymptome();
    handleClose();
  };
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Sexuellbezogene Symptome</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            ...
            {renderCardSection(
              'Sexuelle Funktionsstörungen',
              FormTypes.SexuelleFunktionsstoerungSymptom,
              SEXUELLE_FUNKTIONSSTOERUNG_LABELS,
              'sexuellbezogeneSymptome.funktionsstoerungen',
              sx.funktionsstoerungen,
              expandedFunktion,
              setExpandedFunktion
            )}
            ...
            {renderCardSection(
              'Störungen der Sexualpräferenz',
              FormTypes.SexualpraeferenzstoerungSymptom,
              SEXUALPRAEFERENZSTOERUNG_LABELS,
              'sexuellbezogeneSymptome.praeferenzstoerungen',
              sx.praeferenzstoerungen,
              expandedPraeferenz,
              setExpandedPraeferenz
            )}
            ...
            {(filterBySearch('Andere') || filterBySearch('Symptome')) && (
              <div>
                ...
                <textarea
                  value={sx.andereSymptome}
                  onChange={(e) => setNestedField('sexuellbezogeneSymptome.andereSymptome', e.target.value)}
                  rows={3}
                  placeholder="Weitere sexuellbezogene Symptome..."
