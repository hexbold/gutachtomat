...
interface HyperkinetischeStoerungenModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function HyperkinetischeStoerungenModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: HyperkinetischeStoerungenModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  const [expandedAttentional, setExpandedAttentional] = useState<Set<FormTypes.HyperkinetischAttentionalSymptom>>(new Set());
  const [expandedHyperaktiv, setExpandedHyperaktiv] = useState<Set<FormTypes.HyperkinetischHyperaktivSymptom>>(new Set());
  const [expandedImpulsiv, setExpandedImpulsiv] = useState<Set<FormTypes.HyperkinetischImpulsivSymptom>>(new Set());

  useEffect(() => {
    setMounted(true);
  }, []);

  const hk = formData.hyperkinetischeStoerungen;
  const allSelectedSymptoms = getFormattedHyperkinetischeStoerungen(hk).sort((a, b) => a.localeCompare(b));
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Hyperkinetische Störungen</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            {renderCardSection(
              'Attentionale Symptomatik',
              FormTypes.HyperkinetischAttentionalSymptom,
              HYPERKINETISCH_ATTENTIONAL_LABELS,
              'hyperkinetischeStoerungen.attentional',
              hk.attentional,
              expandedAttentional,
            ...
            {renderCardSection(
              ...
              FormTypes.HyperkinetischHyperaktivSymptom,
              HYPERKINETISCH_HYPERAKTIV_LABELS,
              'hyperkinetischeStoerungen.hyperaktiv',
              hk.hyperaktiv,
              expandedHyperaktiv,
              setExpandedHyperaktiv
            )}

            {renderCardSection(
              'Impulsive Symptomatik',
              FormTypes.HyperkinetischImpulsivSymptom,
              HYPERKINETISCH_IMPULSIV_LABELS,
              'hyperkinetischeStoerungen.impulsiv',
              hk.impulsiv,
              expandedImpulsiv,
              setExpandedImpulsiv
            )}
            ...
            {(filterBySearch('Andere') || filterBySearch('Symptome')) && (
              <div>
                ...
                <textarea
                  value={hk.andereSymptome}
                  onChange={(e) => setNestedField('hyperkinetischeStoerungen.andereSymptome', e.target.value)}
                  rows={3}
                  placeholder="Weitere hyperkinetische Symptome..."
