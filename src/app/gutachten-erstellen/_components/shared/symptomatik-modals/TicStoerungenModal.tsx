...
interface TicStoerungenModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function TicStoerungenModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: TicStoerungenModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  const [expandedMotorisch, setExpandedMotorisch] = useState<Set<FormTypes.MotorischeTicsSymptom>>(new Set());
  const [expandedVokal, setExpandedVokal] = useState<Set<FormTypes.VokaleTicsSymptom>>(new Set());

  useEffect(() => {
    setMounted(true);
  }, []);

  const ts = formData.ticStoerungen;
  const allSelectedSymptoms = getFormattedTicStoerungen(ts).sort((a, b) => a.localeCompare(b));
  ...
  const resetAllExpanded = () => {
    setExpandedMotorisch(new Set());
    setExpandedVokal(new Set());
  };
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Tic-Störungen</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            {renderCardSection(
              'Motorische Tics',
              FormTypes.MotorischeTicsSymptom,
              MOTORISCHE_TICS_LABELS,
              'ticStoerungen.motorischeTics',
              ts.motorischeTics,
              expandedMotorisch,
              setExpandedMotorisch
            )}

            {renderCardSection(
              'Vokale Tics',
              FormTypes.VokaleTicsSymptom,
              VOKALE_TICS_LABELS,
              'ticStoerungen.vokaleTics',
              ts.vokaleTics,
              expandedVokal,
              setExpandedVokal
            )}

            {(filterBySearch('Tourette') || filterBySearch('Syndrom')) && (
              <div>
                <h3 className="text-lg font-bold text-gray-900 mb-4">{highlight('Tourette-Syndrom')}</h3>
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => setNestedField('ticStoerungen.touretteSyndrom', !ts.touretteSyndrom)}
                    className={`relative p-3 rounded-lg border-2 transition-all duration-200 text-left min-h-[60px] flex items-center ${ts.touretteSyndrom ? 'border-blue-500 bg-blue-50 shadow-sm' : 'border-gray-300 bg-white hover:border-blue-300 hover:bg-blue-50/30'} focus:outline-none focus:ring-2 focus:ring-blue-400 cursor-pointer`}
                  >
                    {ts.touretteSyndrom && (
                      <div className="absolute top-1.5 right-1.5 w-4 h-4 bg-blue-500 rounded-full flex items-center justify-center">
                        <svg className="w-2.5 h-2.5 text-white" fill="none" strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <span className={`text-sm font-semibold leading-snug pr-6 ${ts.touretteSyndrom ? 'text-blue-700' : 'text-gray-700'}`}>
                      Tourette-Syndrom vorhanden
                    </span>
                  </button>
                </div>
              </div>
            ...
            {(filterBySearch('Andere') || filterBySearch('Symptome')) && (
              <div>
                ...
                <textarea
                  ...
                  onChange={(e) => setNestedField('ticStoerungen.andereSymptome', e.target.value)}
                  rows={3}
                  placeholder="Weitere Tic-Symptome..."
