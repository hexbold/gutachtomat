...
import { VERHALTENSEXZESS_SYMPTOM_LABELS } from '@/lib/core/form-labels';
import { getFormattedVerhaltensexzesse } from '@/lib/utils/verhaltensauffaelligkeiten-counter';
import { SymptomHandlers } from '@/hooks/useGutachtenForm';
import { useState, useEffect } from 'react';
import { createPortal } from 'react-dom';

interface VerhaltensexzesseModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function VerhaltensexzesseModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: VerhaltensexzesseModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  // Expanded state for CardSelection pattern
  const [expandedExzesse, setExpandedExzesse] = useState<Set<FormTypes.VerhaltensexzessSymptom>>(new Set());
  ...
  const allSelectedSymptoms = getFormattedVerhaltensexzesse(va).sort((a, b) => a.localeCompare(b));
  ...
  const handleCardToggle = (symptom: FormTypes.VerhaltensexzessSymptom) => {
    const currentSelection = va.exzesse;
    const newSelection = { ...currentSelection };
    if (newSelection[symptom]) {
      delete newSelection[symptom];
      // Close expanded if deselected
      if (expandedExzesse.has(symptom)) {
        const newExpanded = new Set(expandedExzesse);
        newExpanded.delete(symptom);
        setExpandedExzesse(newExpanded);
      }
    } else {
      newSelection[symptom] = { selected: true, details: {} };
    }
    setNestedField('verhaltensauffaelligkeiten.exzesse', newSelection as unknown as Record<string, unknown>);
  };

  // Details change handler (brackets only)
  const handleDetailsChange = (
    symptom: FormTypes.VerhaltensexzessSymptom,
    value: string
  ) => {
    const newSelection = {
      ...va.exzesse,
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Verhaltensexzesse</h2>
        ...
        <div className="px-6 pt-4 pb-2">
          <div className="relative">
            <input
              ...
              placeholder="Verhaltensexzesse durchsuchen..."
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            {/* Verhaltensexzesse */}
            {Object.values(FormTypes.VerhaltensexzessSymptom).some(v => filterBySearch(VERHALTENSEXZESS_SYMPTOM_LABELS[v])) && (
              <div>
                <div className="grid grid-cols-2 gap-2.5">
                  {Object.values(FormTypes.VerhaltensexzessSymptom)
                    .filter(v => filterBySearch(VERHALTENSEXZESS_SYMPTOM_LABELS[v]))
                    .map((symptom) => {
                      const isItemSelected = !!va.exzesse[symptom]?.selected;
                      const isExpanded = expandedExzesse.has(symptom);
                      const details = va.exzesse[symptom]?.details;
                        ...
                        <div key={symptom} className="flex flex-col gap-1.5">
                          <div className="flex gap-1.5">
                            <button
                              ...
                              title={VERHALTENSEXZESS_SYMPTOM_LABELS[symptom]}
                              className={`
                            ...
                            >
                              ...
                              <span className={`text-sm font-semibold leading-snug pr-6 line-clamp-2 ${isItemSelected ? 'text-blue-700' : 'text-gray-700'}`}>
                                {highlight(VERHALTENSEXZESS_SYMPTOM_LABELS[symptom])}
                              </span>
                            </button>
                            {isItemSelected && (
                              <button
            ...
            {/* Andere Exzesse - Text Field */}
            <div>
              <label className="block text-lg font-bold text-gray-900 mb-3">
                Weitere Exzesse
              </label>
              <textarea
                value={va.andereExzesse}
                onChange={(e) => setNestedField('verhaltensauffaelligkeiten.andereExzesse', e.target.value)}
                rows={4}
