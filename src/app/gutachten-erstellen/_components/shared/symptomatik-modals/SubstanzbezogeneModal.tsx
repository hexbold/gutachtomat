...
interface SubstanzbezogeneModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function SubstanzbezogeneModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: SubstanzbezogeneModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  // Expanded states for CardSelection sections
  const [expandedAbhaengigkeit, setExpandedAbhaengigkeit] = useState<Set<FormTypes.SubstanzAbhaengigkeitSymptom>>(new Set());
  const [expandedSomatovegetativ, setExpandedSomatovegetativ] = useState<Set<FormTypes.SubstanzSomatovegetativSymptom>>(new Set());
  const [expandedPsychomotorik, setExpandedPsychomotorik] = useState<Set<FormTypes.SubstanzPsychomotorikSymptom>>(new Set());
  const [expandedVerhalten, setExpandedVerhalten] = useState<Set<FormTypes.SubstanzVerhaltenSymptom>>(new Set());
  const [expandedEmotional, setExpandedEmotional] = useState<Set<FormTypes.SubstanzEmotionalSymptom>>(new Set());
  const [expandedSchlaf, setExpandedSchlaf] = useState<Set<FormTypes.SubstanzSchlafSymptom>>(new Set());
  const [expandedNeurologisch, setExpandedNeurologisch] = useState<Set<FormTypes.SubstanzNeurologischSymptom>>(new Set());
  const [expandedKognitiv, setExpandedKognitiv] = useState<Set<FormTypes.SubstanzKognitivSymptom>>(new Set());
  const [expandedPsychotisch, setExpandedPsychotisch] = useState<Set<FormTypes.SubstanzPsychotischSymptom>>(new Set());
  const [expandedDissociativ, setExpandedDissociativ] = useState<Set<FormTypes.SubstanzDissociativSymptom>>(new Set());

  useEffect(() => {
    setMounted(true);
  }, []);

  const ss = formData.substanzbezogeneSymptomatik;

  // Use shared utility for footer display
  const allSelectedSymptoms = getFormattedSubstanzbezogene(ss).sort((a, b) => a.localeCompare(b));
  ...
  const resetAllExpanded = () => {
    setExpandedAbhaengigkeit(new Set());
    ...
    setExpandedSchlaf(new Set());
    setExpandedNeurologisch(new Set());
    setExpandedKognitiv(new Set());
    setExpandedPsychotisch(new Set());
    setExpandedDissociativ(new Set());
  };
  ...
  const handleClearAndClose = () => {
    symptomHandlers.clearSubstanzbezogeneSymptomatik();
    handleClose();
  };
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Substanzbezogene Symptomatik</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            ...
            {renderCardSection(
              'Abhängigkeitssymptome',
              FormTypes.SubstanzAbhaengigkeitSymptom,
              SUBSTANZ_ABHAENGIGKEIT_LABELS,
              'substanzbezogeneSymptomatik.abhaengigkeit',
              ss.abhaengigkeit,
              expandedAbhaengigkeit,
              setExpandedAbhaengigkeit
            )}

            {/* Somatovegetative Symptome */}
            {renderCardSection(
              'Somatovegetative Symptome',
              FormTypes.SubstanzSomatovegetativSymptom,
              SUBSTANZ_SOMATOVEGETATIV_LABELS,
              'substanzbezogeneSymptomatik.somatovegetativ',
              ss.somatovegetativ,
            ...
            {renderCardSection(
              'Psychomotorik',
              FormTypes.SubstanzPsychomotorikSymptom,
              SUBSTANZ_PSYCHOMOTORIK_LABELS,
              'substanzbezogeneSymptomatik.psychomotorik',
              ss.psychomotorik,
              expandedPsychomotorik,
              setExpandedPsychomotorik
            )}

            {/* Verhalten */}
            {renderCardSection(
              'Verhalten',
              FormTypes.SubstanzVerhaltenSymptom,
              SUBSTANZ_VERHALTEN_LABELS,
              'substanzbezogeneSymptomatik.verhalten',
              ss.verhalten,
              expandedVerhalten,
            ...
            {renderCardSection(
              'Emotionales Erleben',
              FormTypes.SubstanzEmotionalSymptom,
              SUBSTANZ_EMOTIONAL_LABELS,
              'substanzbezogeneSymptomatik.emotionalesErleben',
              ss.emotionalesErleben,
              expandedEmotional,
            ...
            {renderCardSection(
              'Schlaf',
              FormTypes.SubstanzSchlafSymptom,
              SUBSTANZ_SCHLAF_LABELS,
              'substanzbezogeneSymptomatik.schlaf',
              ss.schlaf,
              expandedSchlaf,
              setExpandedSchlaf
            )}
            ...
            {renderCardSection(
              'Neurologische Symptome',
              FormTypes.SubstanzNeurologischSymptom,
              SUBSTANZ_NEUROLOGISCH_LABELS,
              'substanzbezogeneSymptomatik.neurologisch',
              ss.neurologisch,
              expandedNeurologisch,
              setExpandedNeurologisch
            )}

            {/* Kognition */}
            {renderCardSection(
              'Kognition',
              FormTypes.SubstanzKognitivSymptom,
              SUBSTANZ_KOGNITIV_LABELS,
              'substanzbezogeneSymptomatik.kognition',
              ss.kognition,
              expandedKognitiv,
              setExpandedKognitiv
            )}
            ...
            {renderCardSection(
              'Psychotische Symptome',
              FormTypes.SubstanzPsychotischSymptom,
              SUBSTANZ_PSYCHOTISCH_LABELS,
              'substanzbezogeneSymptomatik.psychotisch',
              ss.psychotisch,
              expandedPsychotisch,
              setExpandedPsychotisch
            )}

            {/* Dissoziative Symptome */}
            {renderCardSection(
              'Dissoziative Symptome',
              FormTypes.SubstanzDissociativSymptom,
              SUBSTANZ_DISSOCIATIV_LABELS,
              'substanzbezogeneSymptomatik.dissociativ',
              ss.dissociativ,
              expandedDissociativ,
              setExpandedDissociativ
            )}
            ...
            {filterBySearch('Andere') && (
              <div>
                ...
                <textarea
                  value={ss.andereSymptome}
                  onChange={(e) => setNestedField('substanzbezogeneSymptomatik.andereSymptome', e.target.value)}
                  ...
                  placeholder="Weitere substanzbezogene Symptome beschreiben..."
                />
              </div>
            )}
          </div>
