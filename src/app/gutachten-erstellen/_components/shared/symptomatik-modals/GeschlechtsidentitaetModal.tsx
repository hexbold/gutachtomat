...
interface GeschlechtsidentitaetModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function GeschlechtsidentitaetModal({
  isOpen,
  onClose,
  formData,
  symptomHandlers,
  setNestedField
}: GeschlechtsidentitaetModalProps) {
  const [mounted, setMounted] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');

  // Expanded states for CardSelection sections
  const [expandedSymptome, setExpandedSymptome] = useState<Set<FormTypes.GeschlechtsidentitaetSymptom>>(new Set());

  useEffect(() => {
    setMounted(true);
  }, []);

  const gi = formData.geschlechtsidentitaet;

  // Use shared utility for footer display
  const allSelectedSymptoms = getFormattedGeschlechtsidentitaet(gi).sort((a, b) => a.localeCompare(b));
  ...
  const handleClearAndClose = () => {
    symptomHandlers.clearGeschlechtsidentitaet();
    handleClose();
  };
    ...
    <div className="fixed top-36 bottom-4 left-4 right-[46%] z-40 flex items-start justify-center p-2">
      <div className="bg-white rounded-xl shadow-2xl border-2 border-blue-200 w-full max-w-5xl max-h-full overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 bg-gradient-to-r from-blue-50 to-blue-100 border-b-2 border-blue-200 flex items-center justify-between">
          <h2 className="text-2xl font-bold text-blue-900">Geschlechtsidentitätsbezogene Symptomatik</h2>
        ...
        <div className="overflow-y-auto flex-1 px-6 py-4">
          <div className="space-y-8">
            {/* Geschlechtsidentitätsbezogene Symptome */}
            {renderCardSection(
              'Geschlechtsidentitätsbezogene Symptome',
              FormTypes.GeschlechtsidentitaetSymptom,
              GESCHLECHTSIDENTITAET_LABELS,
              'geschlechtsidentitaet.symptome',
              gi.symptome,
              expandedSymptome,
              setExpandedSymptome
            )}
            ...
            {(filterBySearch('Andere') || filterBySearch('Symptome')) && (
              <div>
                ...
                <textarea
                  value={gi.andereSymptome}
                  onChange={(e) => setNestedField('geschlechtsidentitaet.andereSymptome', e.target.value)}
                  rows={3}
                  placeholder="Weitere geschlechtsidentitätsbezogene Symptome..."
